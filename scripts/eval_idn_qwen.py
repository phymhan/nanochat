"""
IDN layer-parallel inference evaluation for Qwen 2.5 0.5B.

Ports the IDN (Identity Newton) evaluation from nanochat to a standard
HuggingFace Qwen model. Qwen uses standard residuals (no hyperconnections,
no x0-resid, no VE), so IDN_batched == ChunkB_NxF1.

Methods: Sequential, IDN batched, Fused, Chunkwise batched
Init strategies: h0, batch_fwd (warm/preheat supported but not swept)
Evaluations: PPL (wikitext-2), timing

Usage:
    HF_HOME=/mnt/nvme0n1/ligong/huggingface CUDA_VISIBLE_DEVICES=4 \
        uv run --extra gpu --group dev python -m scripts.eval_idn_qwen --n-par 8
"""
...
class QwenPrecomputedWeights:
    ...
    def __init__(self, model, par):
        ...
        self.b_q = torch.stack([l.self_attn.q_proj.bias for l in layers]).to(dt)
        self.b_k = torch.stack([l.self_attn.k_proj.bias for l in layers]).to(dt)
        self.b_v = torch.stack([l.self_attn.v_proj.bias for l in layers]).to(dt)
...
class QwenChunkWeights:
    ...
    def __init__(self, model, chunk_layers):
        ...
        self.b_q = torch.cat([l.self_attn.q_proj.bias for l in layers], dim=0).to(dt)
        self.b_k = torch.cat([l.self_attn.k_proj.bias for l in layers], dim=0).to(dt)
        self.b_v = torch.cat([l.self_attn.v_proj.bias for l in layers], dim=0).to(dt)
...
def _batched_forward(all_h_prev, position_embeddings, pw, config):
    """Batched forward for n_par Qwen layers. Returns (n_par, B, T, D)."""
...
def forward_idn_batched(model, input_ids, seq_layers, pw, K=1, elk_k=0.0,
                         init='h0'):
    """IDN batched: seq prefix + batched parallel layers + IDN correction."""
...
def forward_fused(model, input_ids, seq_layers, chunk_weights, K=1, elk_k=0.0,
                   avg=False):
    """Fully fused: seq prefix + single mega-block (no per-layer correction)."""
    h_init, pos_emb = _run_prefix(model, input_ids, seq_layers)
...
def main():
    ...
    pw = QwenPrecomputedWeights(model, par)

    # Chunk configs: N from {n_par, n_par/2, n_par/4, 4, 2}
    chunk_configs = {}
    chunk_configs[f'1xF{n_par}'] = [QwenChunkWeights(model, par)]
    chunk_configs[f'{n_par}xF1'] = [QwenChunkWeights(model, [li]) for li in par]
    ...
    for N in sorted(candidate_N):
        ...
        if key not in chunk_configs:
            chunk_configs[key] = [
                QwenChunkWeights(model, par[i * M:(i + 1) * M]) for i in range(N)
            ]
    ...
    # Save
    outpath = args.output or f'cache/eval_logs/idn_qwen_npar{n_par}.json'
    ...
    # Print summary table
    print(f"\n{'=' * 70}")
    print(f"Qwen IDN Eval: n_par={n_par} (seq PPL={seq_ppl:.2f}, seq_ms={seq_ms:.1f})")
    print(f"{'=' * 70}")
    for K in K_values:
        k_key = f'K={K}'
